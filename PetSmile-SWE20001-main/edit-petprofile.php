...
if(isset($_GET['delete'])){
    $id = $_GET['delete'];
    mysqli_query($con, "DELETE FROM pet WHERE id = $id");
    header('location:edit-petprofile.php');
 }; 
?>
    ...
    <form method="POST" action="edit-petprofile.php">
        ...
        <?php
        $select = mysqli_query($con, "SELECT * FROM pet WHERE Cust_ID='$Cust_ID'");
        while($row = mysqli_fetch_assoc($select)){ 
        ?>
        <a href="edit-petprofile.php?delete=<?= $row['id']; ?>" class="delete-btn btn btn-lg btn-primary px-3 mt-3" onclick="return confirm('Do you want to delete pet?');">Delete</a>
        <?php } ?>
    </form>
